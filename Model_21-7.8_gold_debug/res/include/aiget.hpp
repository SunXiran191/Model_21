            ...
            if (j.contains("objects") && j["objects"].is_array())
            {
                for (const auto &obj : j["objects"])
                {
                    ...
                    if (!obj.contains("x1") || !obj.contains("y1") ||
                        ...
                        (!obj.contains("score") && !obj.contains("confidence")) ||
                        !obj.contains("class_id"))
                    {
                        continue;
                    }

                    PredictResult det{};
                    det.x1 = static_cast<int>(obj["x1"].get<float>());
                    det.y1 = static_cast<int>(obj["y1"].get<float>());
                    det.x2 = static_cast<int>(obj["x2"].get<float>());
                    det.y2 = static_cast<int>(obj["y2"].get<float>());
                    ...
                    {
                        det.score = obj["confidence"].get<float>();
                    }
