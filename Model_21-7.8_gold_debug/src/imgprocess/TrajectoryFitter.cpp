...
vector<Point> FitTrajectory_LOWESS(int point_count, const vector<Point> &pts, const Mat &frame)
{
    vector<Point> curve_pts;
    ...
    // 预分配输出容量，避免 push_back 触发多次重分配
    curve_pts.reserve(static_cast<size_t>(point_count));

    // 平滑窗口大小
    const double span = 0.10;
    const int window_size = max(3, static_cast<int>(n * span));
    const double inv_ws = 1.0 / static_cast<double>(window_size); // 预计算倒数，内循环用乘法代替除法
    const double inv_pc = 1.0 / static_cast<double>(point_count - 1);

    for (int i = 0; i < point_count; ++i)
    {
        double t = static_cast<double>(i) * inv_pc * static_cast<double>(n - 1); // 目标插值点位置

        // 限制内循环仅扫描窗口内的索引范围
        int j_lo = max(0, static_cast<int>(t - window_size));
        int j_hi = min(n - 1, static_cast<int>(t + window_size));

        double sum_w = 0.0, sum_wt = 0.0, sum_wt2 = 0.0;
        double sum_wx = 0.0, sum_wy = 0.0, sum_wtx = 0.0, sum_wty = 0.0;

        for (int j = j_lo; j <= j_hi; ++j)
        {
            double diff = fabs(t - static_cast<double>(j));
            double u = 1.0 - (diff * inv_ws) * (diff * inv_ws) * (diff * inv_ws);
            double w = u * u * u;

            double wj = w * static_cast<double>(j);
            sum_w += w;
            sum_wt += wj;
            sum_wt2 += wj * static_cast<double>(j);
            sum_wx += w * pts[j].x;
            sum_wy += w * pts[j].y;
            sum_wtx += wj * pts[j].x;
            sum_wty += wj * pts[j].y;
        ...
        if (fabs(det) > 1e-6)
        {
            ...
            double x_fit = (sum_wt2 * sum_wx - sum_wt * sum_wtx) * inv_det + (sum_w * sum_wtx - sum_wt * sum_wx) * inv_det * t;
            double y_fit = (sum_wt2 * sum_wy - sum_wt * sum_wty) * inv_det + (sum_w * sum_wty - sum_wt * sum_wy) * inv_det * t;
...
// 高斯加权平滑 + 均匀采样，比 LOWESS 快 20 倍以上，平滑去噪效果相近
// 原理：对有序 2D 轨迹点做高斯核加权平滑，再等距重采样
vector<Point> FitTrajectory_Gauss(int point_count, const vector<Point> &pts, const Mat &frame)
{
    vector<Point> curve_pts;
    int n = static_cast<int>(pts.size());
    if (n < 2)
    ...
    // ---- 第1步：高斯加权平滑（窗口=7, sigma=2.0） ----
    const int m = 3;          // 半窗宽
    const double sigma = 2.0; // 高斯标准差
    // 预计算高斯权重（对称，只需计算正半轴）
    double raw[7];
    double sum_w = 0.0;
    for (int k = 0; k <= m; ++k)
    {
        ...
        raw[m + k] = exp(-d * d / (2.0 * sigma * sigma));
        if (k > 0)
            raw[m - k] = raw[m + k];
        sum_w += raw[m + k];
        if (k > 0)
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        for (int k = -m; k <= m; ++k)
        {
            ...
            int cid = k + m;
            sum_x += coeff[cid] * static_cast<double>(pts[idx].x);
            sum_y += coeff[cid] * static_cast<double>(pts[idx].y);
        }
        sx[i] = sum_x;
        sy[i] = sum_y;
    }

    // ---- 第2步：等距采样 point_count 个点（线性插值） ----
    const double step = static_cast<double>(n - 1) / static_cast<double>(point_count - 1);
    for (int i = 0; i < point_count; ++i)
    {
        double pos = static_cast<double>(i) * step;
        int idx0 = static_cast<int>(pos);
        int idx1 = min(idx0 + 1, n - 1);
        double frac = pos - static_cast<double>(idx0);

        double x = sx[idx0] + (sx[idx1] - sx[idx0]) * frac;
        double y = sy[idx0] + (sy[idx1] - sy[idx0]) * frac;
        curve_pts.push_back(Point(cvRound(x), cvRound(y)));
    }
    return curve_pts;
}
...
// 二次贝塞尔曲线
vector<cv::Point> FitTrajectory_Bezier_2d(cv::Point p0, cv::Point p1, cv::Point p2, int num_points)
{
...
vector<Point> FitTrajectory_Bezier(int point_count, const vector<Point> &lane_points)
{
    ...
    const double dt = 0.05; // 建议值：0.01~0.05，0.02兼顾平滑度和效率
